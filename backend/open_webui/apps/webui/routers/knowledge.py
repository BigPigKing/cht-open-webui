...
@router.post("/{id}/async/file/add", response_model=Optional[KnowledgeFilesResponse])
async def aadd_file_to_knowledge_by_id(
    id: str,
    form_data: KnowledgeFileIdForm,
    user=Depends(get_admin_user),
):
    ...
    # Add content to the vector database
    try:
        await aprocess_file(ProcessFileForm(file_id=form_data.file_id, collection_name=id))
    except Exception as e:
        log.debug(e)
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
@router.post("/{id}/async/file/update", response_model=Optional[KnowledgeFilesResponse])
async def aupdate_file_from_knowledge_by_id(
    id: str,
    form_data: KnowledgeFileIdForm,
    user=Depends(get_admin_user),
):
