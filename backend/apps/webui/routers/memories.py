...
from config import SRC_LOG_LEVELS, RAG_VECTORSTORE_CLIENT
...
async def add_memory(
    ...
    collection = RAG_VECTORSTORE_CLIENT.get_or_create_collection(name=f"user-memory-{user.id}")
    collection.upsert(
...
async def update_memory_by_id(
    ...
    if form_data.content is not None:
        ...
        collection = RAG_VECTORSTORE_CLIENT.get_or_create_collection(
...
async def reset_memory_from_vector_db(
    ...
    RAG_VECTORSTORE_CLIENT.delete_collection(f"user-memory-{user.id}")
    collection = RAG_VECTORSTORE_CLIENT.get_or_create_collection(name=f"user-memory-{user.id}")
